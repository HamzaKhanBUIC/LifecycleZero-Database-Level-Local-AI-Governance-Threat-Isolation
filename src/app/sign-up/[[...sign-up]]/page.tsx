...
import { Shield, Sparkles, Loader2, UserPlus } from "lucide-react";
            ...
            <form onSubmit={handleMockRegister} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5 mb-2">
                <div className="flex items-center gap-1.5 text-xs text-yellow-500 font-bold tracking-wider uppercase">
                  ...
                  <span>Sandbox Registration Active</span>
                </div>
                <h1 className="text-white text-base font-bold uppercase tracking-tight">Create Demo Workspace</h1>
                <p className="text-zinc-500 text-[10px] uppercase leading-relaxed">
                  Enter details to register a mock tenant profile and provision your local sandbox assets partition.
                </p>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-zinc-400 text-[9px] uppercase tracking-wider font-bold">Full Name</label>
                <input 
                  type="text" 
                  value={fullName}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-zinc-400 text-[9px] uppercase tracking-wider font-bold">Organization Name</label>
                <input 
                  type="text" 
                  value={orgName}
                  onChange={(e) => setOrgName(e.target.value)}
                  placeholder="e.g. Sector-7 Labs"
                  className="bg-zinc-950 border border-zinc-850 focus:border-zinc-500 rounded-sm text-zinc-300 font-mono p-2.5 text-xs outline-none"
                  required
                />
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-zinc-400 text-[9px] uppercase tracking-wider font-bold">Business Email</label>
                <input 
                  type="email" 
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="e.g. security@sector7.com"
                  className="bg-zinc-950 border border-zinc-850 focus:border-zinc-500 rounded-sm text-zinc-300 font-mono p-2.5 text-xs outline-none"
                  required
                />
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <span>Provision & Launch Demo</span>
              ...
              <div className="text-center mt-3 pt-3 border-t border-zinc-900">
                <span className="text-zinc-500 text-[10px] uppercase">
                  ...
                  <Link href="/sign-in" className="text-white hover:underline underline-offset-4 font-bold">
                    Log In
                  </Link>
                </span>
              </div>
